...
public class PositionProvider {
    ...
    public static List<BlockPos> getResults(BlockPos pistonPos, EnumFacing facing, BlockPos tntPos) {
                ...
                for (int l = 0; l < 16; ++l) {
                    if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
                        ...
                        for (float f = size * (0.7f + 0.6f * rng.nextFloat()); f > 0.0f; f -= 0.22500001) {
                            ...
                            IFluidState ifluidstate = mc.world.getFluidState(blockpos);
                            if (j == 13 && k == 15 && l == 11) {
                                System.nanoTime();
                            }
                            if (!blockpos.equals(tntPos) && (!iblockstate.isAir() || !ifluidstate.isEmpty())) {
                                float resistance = Math.max(iblockstate.getBlock().getExplosionResistance(), ifluidstate.getExplosionResistance());
